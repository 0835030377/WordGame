...
public class WordgameServiceTests {
	@Mock
	private IDictionaryProvider dictionaryProvider;

	@Mock
	private WordgameStrategy wordgameStrategy;
	...
	@Before
	public void setup() {		
		service = new WordgameService(Arrays.asList(dictionaryProvider), 
				Arrays.asList(wordgameStrategy));
	}
	...
	private void givenAValidLevel(int level) {
		...
		Mockito.when(wordgameStrategy.isStrategyForLevel(level)).thenReturn(true);
	}
	...
	private void givenSessionBean() {
		String game = "wordgame";
		Map<String, String> properties = new HashMap<String, String>();
		if (currentString != null) {
			properties.put("wordgame_string", currentString);
		}
	...
	private void givenStrategyLetterResponseFor(String string, String letter) {
		Mockito.when(wordgameStrategy.addLetter(any(List.class), eq(string))).thenReturn(letter);
	}
	...
	private void givenStrategyReturns(String letter) {
		Mockito.when(wordgameStrategy.addLetter(any(List.class), any(String.class))).thenReturn(letter);
	}
