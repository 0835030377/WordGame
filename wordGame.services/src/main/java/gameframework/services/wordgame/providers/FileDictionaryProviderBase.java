...
public abstract class FileDictionaryProviderBase implements IDictionaryProvider {
	...
	private static final Logger LOG = Logger
			.getLogger(FileDictionaryProviderBase.class);
	...
	/**
	 * Initializes the dictionary list. Currently, the list of words will load
	 * in memory, but if there are other many dictionary, review this load
	 * mechanism in order to avoid out of memory exceptions. The words with less
	 * than the minimal letters will be discarded. Also the ones that starts
	...
	public FileDictionaryProviderBase(String dictionaryFile, int minimalLetters) {
		...
		try {
			InputStream is = this.getClass()
					.getResourceAsStream(dictionaryFile);
			...
			while (word != null) {
				if (word.length() > minimalLetters
						&& (previousWord == null || !word
								.startsWith(previousWord))) {
			...
			LOG.info(String.format("%s words in dictionary %s",
					this.wordList.size(), dictionaryFile));

		} catch (Exception ex) {
			LOG.error("Error reading dictionary", ex);
		} finally {
	...
	public List<String> listPossibleWords(String string) {
		...
		LOG.debug(String.format("%s words for string %s", words.size(), string));
